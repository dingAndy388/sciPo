...
| E20 | 敌方死亡掉落资源入池 | ⚠️ | L：无掉落、无死亡事件（`UNIT-08`） | ✔（新增事件） | 配置加 `Loot` + 死亡处理 — **小~中** ｜ **✅ 事件已就位**（v0.3.14 / `WP-2.10`），**掉落入池归 `WP-3.7`**（`DropReward` 已填表未消费） |
| E21 | 单位死亡：移除/不返还/驻扎加成消失 | ⚠️ | L：移除已有（`:325-332`），缺驻扎清理与事件 | ✔ | 补齐清理 — **中** ｜ **✅ 移除 + 不返还 + 事件闭环（v0.3.22 / WP-3.6）**：交战槽位/索引/任务/交战循环一并清理；**驻扎加成清理归 `WP-3.7`**（驻扎系统本身在 `WP-4.6`） |
...
| WP-3.7 | 单位死亡：移除 / 不返还 / 掉落入池 / 清理驻扎 | `E20/E21` | `UnitCombatService` | `Loot` 配置字段 | 中 | 代码 | → **部分前置已就位（v0.3.22）**：移除 + 不返还 + 阵亡事件由 `WP-3.6` 的 `KillUnit` 承担；待做 = 消费 `DropReward` 进资源池（`E20`）+ 驻扎加成清理 |
...
| M0-3 | ③ 同格交战 N 日后 HP 归零、单位移除、掉落进池 | ⏳ `WP-3.6` ✅ 前两半（v0.3.22）/ `WP-3.7` 待掉落 |
...
当前验收结果（2026-09-15，**166/166 通过、退出码 0**）：M0-1 组 43 项 + M0-2 组 60 项 + M0-3 及后续工作包（`WP-3.1`~`WP-3.10`，逐组见下表；`WP-3.8` 行随 v0.3.20 回填、`WP-3.10` 行随 v0.3.21 增补、`WP-3.6` 行随 v0.3.22 增补）。总数以载具输出为准（下表为覆盖摘要）。
...
| 同格战斗（WP-3.6） | 11 | 伤害口径纯函数（`0.5×1.5=0.75` / 单位不衰减 / 弩炮 18.75）/ **M0-3 ③ 前两半**：近战攻进敌格（同格一对、第二人被拒、攻击不耗 MP、战斗中不算空闲）/ 按日双向结算（8 与 6 点、第 8 日一方阵亡、循环全部注销）/ 击杀后被挑战方的位置由胜者接管 + 封锁解除 + 阵亡事件 + **资源池不变（掉落归 `WP-3.7`）** / 远程隔格（不共格、近战敌人打不到）+ 射程外先接近再开火 / `E19` 进入射程即受击且**不再"遇敌即停"**、撤出射程后循环自行注销 / 建筑可瞄准·×0.5·`UnitAttack +50% → 4.5`·无友伤 / `MapSave` v3 的 `Invader` 槽位跨档保留 + v2 旧档按未交战读回 / 读档恢复双向交战循环（敌方反击不消失）/ 0 伤害单位保持封锁且不白挂循环 / 2 条配置 warning 口径 |
...
## 18.6 当前进度与下一步（v0.3.22 追加）

**M0-3 进度**：① 存档等价 ✅（`WP-3.2`/`3.3`）② 移动模型 ✅（`WP-3.5`）③ 同格交战 ✅ **前两半**（`WP-3.6`：同格交战 → 按日结算 → HP 归零 → 单位移除 + 阵亡事件；**掉落进池待 `WP-3.7`**）④ 敌方封锁格不可建造 ✅（`WP-3.8`）。

**下一步 `WP-3.7`（击杀 / 掉落接线，量级"中"）** —— 验收目标 = M0-3 ③ 整条闭合：

1. 消费已填好的 `IUnitConfig.DropReward`（野狼 30 Gold / 野猪 80 Gold + 20 Wood / 山鹰 50 Gold / 巨角山羊 150 Wood + 50 Idea / 巨鳄 80 Gold + 40 Wood）。挂在哪一侧要想清楚：战斗引擎（Units）**不认识资源池**，因此料想仍走"事件 + 消费方"（`UnitDiedEvent` 已带 `OwnerId`/`UnitUId`/`UnitId`/`Position`/`KillerUId`，且 `WP-4.7`/`WP-4.8` 也会消费它）；备选是新增一条 `ILootSink` 契约（与 `IPopulationSink` 同思路，`D62` 的先例）。
2. 口径草案：只给**玩家击杀敌方**掉落（被杀者 `IsHostile`），掉落进**击杀者所有者**的资源池；玩家单位阵亡不掉落（设计稿"不掉落单位或建筑"）。
3. 若引入 `ILootSink`，同步确认装配位置（`UnitsAppService` 的可选构造参数风格）+ 无头夹具的注入方式（`CombatChecks`/`MonthlySettlementChecks` 的 Harness 风格）。
4. 驻扎加成清理（`E21` 后半）实际依赖驻扎系统（`WP-4.6`）：当前没有驻扎字段，本轮只需记一行"待 `WP-4.6`"，不要为它造一个空接口。
5. 检查项预期 **+3~5**（消费端接线 + 掉落数量/归属 + "玩家阵亡不掉落"的负向对照）。

**之后**：批次 3 其余工作包（见 §17 的工作包表）→ **M0-3 收口 v0.5.0**。
