...
| WP-4.9 | 区域建筑 + 附属建筑嵌套（`MapCell` 建筑集合 + `HostUId`） | ☐ 未开始 | M2 |
| WP-4.10 | **迷雾按 owner 拆分**（状态/服务分离 + 永久清除语义 + 视野列） | ☐ 未开始 | M2 |
| WP-4.11 | 地形通行解锁（水域/山地由科技解锁） | ☐ 未开始 | M2 |
...
| WP-4.16 | 建筑前置建模（设计稿的"前置"含**建筑**：日晷 ← 学院） | ☐ 未开始 | M2 |
| WP-4.17 | 人口模型（聚落级容量：多住房不叠加；拆住房 → 迁移/减员） | ☐ 未开始 | M2 |
...
**进度**：批次 0~3 ✅（29 WP）· 批次 4 **11/19** · 批次 5 **6/11** · 批次 6 **6/6 ✅** · 批次 7 **4/7** · 批次 8 **0/4** → **已完成 56 / 剩余 20**（M2 剩 15 + M3 剩 5）。
