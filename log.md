...
| WP-4.4 | 修正作用面扩展（上限 / 时长 / 视野 / 消耗 / 任务速率） | ◐ 进行中（v0.8.4：已接 `BuildingSpeed`(建造+升级耗时) / `BuildingUpgradeCost` / `ResearchSpeed` / `FoodConsumption`；剩 `UnitTrainingSpeed`、`UnitSpeed`） | M2 |
| WP-4.5 | 产出浮动 ±20% + 观星台改写上下限 | ☐ 未开始 | M2 |
...
**进度**：批次 0~3 ✅（29 WP）· 批次 4 **5/19** · 批次 5 **6/11** · 批次 6 **6/6 ✅** · 批次 7 **4/7** · 批次 8 **0/4** → **已完成 50 / 剩余 26**（M2 剩 21 + M3 剩 5）。
