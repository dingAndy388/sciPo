...
| C9 | 赤字连续 3 年 → 按缺口 logistic 随机减员 | ❌ | L：无 | ✘ | 赤字记录 + 年评估 + 按地块随机减员 — **中~大** ｜ **◐ v0.3.20 / WP-3.9 部分**（赤字缺口 + 连续赤字月数已记录并落盘；年评估与减员归 `WP-3.10`） |
...
> ✅ **已修复（v0.3.20 / `WP-3.9`，主体）**：新增 `MonthlySettlementService`（月边界 → 产出汇总 → 需求 → 扣减 → 赤字记录），需求由 `IUpkeepDemandSource` 实现方上报（人口 / 单位维护两条来源），赤字逐资源记录 + 连续赤字月数并落盘（`settlement:{mapId}`）。仍归 `WP-3.10`：`C9` 的"连续 36 月 → 年评估 → 减员"惩罚、建筑维护、以及 `C4`（产出归因查询 API，`MOD-07`）的 UI 面。
...
> ◐ **部分修复（v0.3.20 / `WP-3.9`）**：`C7`（人口 × N/月）与 `C8`（单位维护）已闭环"需求 → 扣减 → 赤字"，连续赤字月数与逐资源缺口已记录并落盘；**`C9` 的减员尚未实现** —— logistic 年评估 + 按地块随机减员归 `WP-3.10`（它直接消费本次的赤字记录）。另：需求资源目前映射到 `Gold`（原型表暂无 Food，别名口径见 §18.4.2）。
...
当前验收结果（2026-09-15，**147/147 通过、退出码 0**）：M0-1 组 43 项 + M0-2 组 60 项 + M0-3 及后续工作包（`WP-3.1`~`WP-3.9`，逐组见下表；`WP-3.8` 行随 v0.3.20 回填）。
...
| 月度结算器**未进组合根**（v0.3.20） | `MonthlySettlementService` 目前只在无头夹具（`MonthlySettlementChecks` 的 Harness）里装配 —— 与 `UnitsAppService`/`ConstructionAppService`/各仓储同源：`ServiceContainer` 仍未装配任何应用服务 | 真实 Godot 运行时没有月结 → 资源只涨不扣（`TIME-14` 的"闭环"目前只在验收面成立） | `WP-5.1`（应用服务进组合根）前必须补 |
...
| 赤字**没有惩罚**（v0.3.20） | 结算器记录逐资源缺口 + 连续赤字月数并落盘，但没有任何后果 —— 设计稿要求连续 36 月不足 → 年评估 → logistic 随机减员（`C9`） | 长期赤字与吃饱无差别，经济张力只停在账面 | WP-3.10 |
| 建筑维护费未纳入结算（v0.3.20） | 需求来源目前只有两条（人口 / 单位）；设计稿的"建筑维护"（如矿场 / 学院）尚未定义字段 | 建筑只产不耗，中期经济会偏松 | WP-3.10（加一条 `IUpkeepDemandSource` 即可，`D59`） |
