...
| **M0-2 · 单玩家可玩** | 批次 2（`WP-2.1~2.10`） | ① **物理树根节点（简单机械直觉）可研究** → 证明 `TECH-07` 死锁解除；② 建成 School 后 6 个月内存出 250 idea/月；③ 营地 90 日建成 → 人口上限与视野生效；④ 工坊训练工人 30 日后单位出现且人口 −1；⑤ 固定种子下 3 年事件触发次数落在期望区间 | **① ✅ WP-2.1；② ✅ WP-2.7；③ ✅ `WP-2.3`（营地 90 日建成 → 视野生效 + 人口上限 9）；④ ✅ `WP-2.5`（工坊 90 日建成 → 训练工人 30 日 → 单位落位 + 人口 −1）；⑤ ✅ `WP-2.8`（检查 64/64）** |
