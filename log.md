...
| :--- | :--- | :--- | :--- |
| `P-AI-1` | **"60 年"还是"60 日"**？AI.md 写"初始 60 年内不造兵""无威胁 0%（60 年内）/5%（60 年后）" —— 按 1 年 = 360 日，60 年 = 21600 日（一局可能只有 3600 日），字面读会"整局都不造兵" | **按 60 日**（= 2 个月，与"AI 侧重发展、不在前期暴兵"的意图一致）；常量集中在一处，改一个数字即可 | `WP-6.4` 的军队触发 |
...
| `U2` | **73×143 的行列方向**（73 是宽还是高） | `WP-5.8` 已实测：**两种方向的耗时/内存/存档体积几乎一致**（生成 214 vs 218 ms、渲染 958 vs 933 ms、地图文件都是 2.4 MB）—— 也就是说这是**纯观感问题**（哪个方向少滚屏），不是性能问题。当前默认按字面 `宽×高` = 73 列 ×143 行（竖幅世界 26718×45331）；另一个方向 143 列 ×73 行是横幅（52338×23141），更贴合 16:9 显示器。**默认值只在一处**（`DevMapUi.DefaultWidth/DefaultHeight`），换向只改两个数字 | 用户（**N1** 看画面时一句话定即可） | `WP-5.9`（开局布置会顺带把地图规模挪进配置） |
