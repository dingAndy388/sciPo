...
### 批次 4 · 深度机制（19 WP · 完成 3 / 未开始 16）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-4.1 | 修正器升级（多 target 累加 + 宿主作用域 + 阶段管道） | ✅ 完成（v0.8.3：阶段管道 Building→Tech→Event、多 target 累加、宿主作用域 `GetValueForHost`、同宿主去重） | M2 |
| WP-4.2 | 范围效果（观星台 3 格 / 骨笛工坊 1 格 / 振动与波相邻同类） | ☐ 未开始 | M2 |
| WP-4.3 | 单位标签 + 条件化修正（对近战 +20% / 受伤 −25% / 对建筑 +50%） | ☐ 未开始 | M2 |
...
| WP-4.6 | 驻扎系统（学者/化学家：影响范围、产出加成、宿主消失即失效） | ☐ 未开始 | M2 |
| WP-4.7 | 单位合并（同模板、HP 之和 ≤ MaxHP、同格） | ☐ 未开始 | M2 |
...
**进度**：批次 0~3 ✅（29 WP）· 批次 4 **7/19** · 批次 5 **6/11** · 批次 6 **6/6 ✅** · 批次 7 **4/7** · 批次 8 **0/4** → **已完成 52 / 剩余 24**（M2 剩 19 + M3 剩 5）。
