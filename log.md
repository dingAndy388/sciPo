...
| WP-5.8 | **73×143（10439 格）基线**：生成/渲染/存档/迷雾耗时与内存基线（行长口径一并在本 WP 定死） | ☐ 未开始 | M1 |
...
| :--- | :--- | :--- | :--- | :--- |
| `R1` | **73×143 = 10439 格**：存档体积、迷雾矩阵、渲染节点数 | 已实测：生成 **220~310 ms** / 渲染 **~944 ms**（10439 个 `Node2D`+`Sprite2D`，`--headless` 实测），存档与迷雾尚未在这个尺度上跑过 | ① 冒烟固定生成 10439 格（已落地）；② `WP-5.8` 建立耗时/内存基线；③ 渲染节点数在 `WP-5.3`/`5.5` 一并测（若超预算就换 `MultiMesh`/图块合批） | `WP-5.8`、`WP-5.5`、`WP-5.6` |
...
| `U2` | **73×143 的行列方向**（73 是宽还是高） | 当前按字面 `宽×高` 传入生成器（`DevMapUi`: `Width=73 / Height=143`） | 用户（顺带确认）或 `WP-5.8` 复核 | `WP-5.8` |
...
**进度**：批次 0~3 ✅（29 WP）· 批次 4 **1/19** · 批次 5 **5/11** · 批次 6 **0/6** · 批次 7 **0/7** · 批次 8 **0/4** → **已完成 35 / 剩余 41**（到 M2 还差 **M1 的 4 个 + M2 的 33 个**，M3 另 5 个）。
最近一次更新：**v0.6.1**（`WP-5.3` 外观数据驱动交付；检查 **186/186**；Godot 无头冒烟 **15/15、退出码 0** —— 新增"配置表 → 服务 → 表现层"外观比对与占位色渲染）。
