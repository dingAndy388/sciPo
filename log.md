...
| ~~Q1~~ | ~~多玩家形态~~ | **✅ 已关闭（v0.6.0 / D71）**：**单机 + N 个 AI 对手**（不做热座/联机）。AI 与人类共用同一套规则与同一份玩家表（`PlayerContext`），差别只在"谁下指令"（人类 = UI，AI = 决策循环）。因此**不需要**确定性模拟 / 权威服务器 / 回放，但需要"AI 与人类信息对称"（`WP-6.5`） | 已转 `WP-4.18` / 批次 6 |
...
| ~~Q5~~ | ~~是否需要"对象级 buff / 全局环境状态（天气等）"？~~ | **✅ 已关闭（v0.6.0 / D71）**：**不做对象级 buff 宿主化**。修正器继续按 `(mapId, ownerId, target)` 聚合读取（现有 `ModifierAppService` 即够）；"建筑被拆 → 其加成随之消失"由修正器的**注销**保证（`WP-4.8` 建筑 HP/夺取时回收），不引入新的宿主类型。若将来要做"单位自身携带 buff"，届时按 §8.1 的实测流程单独立项 | `MOD-03`、`B8` |
| Q6 | 是否引入成熟 DI 容器，还是坚持手写组合根？ | 决定 `WIRE-01`/`DEP-02` 的实现方式 | `WIRE-01`、`WIRE-06` |
| ~~Q7~~ | ~~是否接受"存档不兼容旧档"（还是需要迁移）？~~ | **✅ 已关闭（v0.6.0 / D71）**：原型期**允许弃档**（不做"为了兼容而兼容"），但**保留迁移链**（`ISaveMigration` + `SaveVersion`，已有 v1→v2→v3 实例）；口径 = 破坏性改动可以升版本号让旧档自然失效，同时**新档必须能一路读到最新**，且每次破坏性改动要写一条迁移或明确登记"本档不兼容" | `DEP-06`、`B4` |
...
| **v0.6.0** | 2026-09-15 | **WP-5.1 装配通电 + WP-4.18 会话/玩家表 + WP-5.2 脚手架修复（M1 起步 · 批次 5 首批）**：① **装配通电（`WP-5.1` / `D74`）**：`CoreBootstrap` 从"只装 Map + Time"补成**全量装配** —— Construction / Units / TechTrees / Events / Resources / Modifiers / Fog / 月结 / 世界存档 / 任务仓储 / 会话编排全部进组合根，并按"有 `ISaveStore` 才接线持久化仓储"的口径收敛（无存档单元 = 无持久化，既有 170 条用例语义不变）；`CoreServices` 增加 13 个只读属性，`ServiceContainer` 启动时打印**装配自检**（缺服务在启动那一刻可见）；② **会话/玩家表（`WP-4.18` / `D78`/`D72`/`D75`/`D80`）**：新增 `PlayerContext`（ownerId ≥ 1 = 势力 / 0 = 中立 / < 0 = 野怪；`IsHuman` 决定是否跑事件引擎）与 `GameSession.Players`（缺省单人类玩家，多势力按 ownerId 升序），新增 **`SessionOrchestrator`**（逐 owner 启动资源池 / 月结 / 事件引擎，幂等、地图不存在则不启动）—— 这是"加一个 AI 不串味"的地基；③ **脚手架修复（`WP-5.2`）**：`MapView` 服务注入（原先 `_mapQuery` 从未赋值 → `UpdateAllCells` 必 NRE）、格子挂到 `MapCells` 容器、单格重建不残留；`DevMapUi` 缺服务时禁用按钮 + 缺省 **73×143（10439 格）** + 生成后相机对中；`CameraController` 重写为**以光标为锚的滚轮缩放 + WASD + 左键拖动**（`WasDragged` 供 M2 的点选短路）并接进 `map_view.tscn`；修正 `service_container.tscn` 的 `Scripts/AutoLoad/` 路径大小写；④ **可验证性**：新增 **`Tests/…/WiringChecks.cs` 10 条断言**（检查 **170→180/180、退出码 0**）与 **`Scene/Dev/smoke_report.tscn` Godot 无头冒烟**（`--smoke` 独立存档；实测 **14/14、退出码 0**：装配自检全 True、73×143 生成 **220~310 ms**、5 条周期任务 = 月结1+事件1+资源成长3、90 日 3 次月结 / 270 次掷骰 / 资源池 Gold=3 Wood=1.5、存档往返恢复 4 条任务、**实例化主场景并渲染 10439 格 ~944 ms**）；⑤ **文档重规划（`D79`/`D71`/`D73`）**：新增 **§19 活文档**（里程碑判据 / 76 个 WP 的批次状态表 / 风险 R1~R7 / 无归属 U1~U6 / 进度与三条复现命令）与 **§20 决策记录（`D71`~`D81`）**，§13/§14 标 `[已归档 · 由 §19 取代]`，§10.1 补入 `design/AI.md` 并新增 **§10.7 AI 规格核对**（10 条可落地口径 + 7 条待澄清），关闭 §8.2 的 Q1/Q5/Q7，修正 3 处记录瑕疵（§11 判定列口径、§18.4.2/§18.7.4 的"科技 6 节点"、§18.5.4 混入的缺陷表行），新增活文档 `Document/TestPlan.md`、`Document/ArtSpec.md`、`Document/AssetManifest.csv` |
...
> 来源：`design/` 下 **6 份**设计稿（v0.6.0 补入 `AI.md`；与仓库内容逐字校验一致）。**10.3 的条目是与设计方确认后的权威口径，后续实现以此为准。**
...
| `design/AI.md` | **AI 对手**（7 段：基础机制 / 发展逻辑 / 决策机制 / 科技机制 / 军事机制 / AI 的限制） | *v0.6.0 补入*：设计稿的实际份数是 **6 份**（本表 v0.2 成文时漏列 `AI.md`）—— 核对结论与待澄清项见 **§10.7**，落地归 **§19 批次 6**（`WP-6.1`~`WP-6.6`） |
...
## 10.7 AI 规格核对与待澄清项（v0.6.0 新增）

> 来源：`design/AI.md`（148 行）。本节把"**已可直接落地**"与"**必须先问用户**"分开：前者进 §19 批次 6 的 WP，后者列在 10.7.2 且**不阻塞**开工（先用合理缺省，等用户答复再调数值/枚举）。

### 10.7.1 已明确、可直接落地的口径（→ 对应 WP）

| # | AI.md 原文口径 | 落地方式 | WP |
| :--- | :--- | :--- | :--- |
| `A-AI-1` | **AI 不面对任何事件**（事件仅对玩家生效） | 事件引擎只对"人类玩家"启动（`SessionOrchestrator` 已实现并有用例锁住） | `WP-4.18` ✅ / `WP-6.5` 复核 |
| `A-AI-2` | **AI 不作弊**：不无视迷雾、不凭空生成单位/资源 | AI 的所有动作都走与玩家相同的应用服务（建造/训练/研究/消耗），没有"直接造单位"的后门；视野必须走**按 owner 的迷雾**读取 | `WP-6.2`、`WP-6.5`、`WP-4.10`（迷雾按 owner 拆分） |
| `A-AI-3` | AI 拥有**独立的资源存量 / 科技进度 / 视野** | 玩家表（`PlayerContext`）为键的资源池 / 科技树 / 迷雾；月结与资源成长都按 owner 各自一份（已实现） | `WP-4.18` ✅ / `WP-4.10` |
| `A-AI-4` | 单位生产**消耗资源与人口**、与建筑绑定、**不瞬生** | 全部走 `UnitsAppService.TrainUnit`（生产队列在建筑上）—— AI 不允许绕过 | `WP-6.4` |
| `A-AI-5` | 建造**受科技前置限制**，可建玩家拥有的基础建筑 | 走 `ConstructionAppService.StartConstruction`（已有的地形/科技/无敌人三项校验自动生效） | `WP-6.3` |
| `A-AI-6` | **军队规模有上限**；维护费会自己压下来；保留最低生产/科研资源 | 军事投入比例有上限 + "资源低于阈值 → 减兵/停兵"；复用单位维护费（`UnitMaintenanceUpkeepDemandSource`） | `WP-6.3`、`WP-6.4` |
| `A-AI-7` | **优先防御**，仅在"高威胁/致命威胁"时主动进攻 | 威胁等级 ≥ 高 才发起进攻指令 | `WP-6.4` |
| `A-AI-8` | 决策 = **固定间隔**执行"生存 → 威胁 → 发展"三级优先级 | 一条按游戏日的 AI tick（间隔可配），决策顺序固定、可复现 | `WP-6.2` |
| `A-AI-9` | **AI 不做的事**：不完美克制玩家科技、不拆自己的经济链、不无限扩张 | 三条当作**测试判据**：随机化决策里禁止"拆自家生产建筑"；扩张有上限；科技选择不看玩家状态 | `WP-6.2`、`WP-6.6` |
| `A-AI-10` | **软肋 = 设计杠杆**：大军 → 科研效率下降；不预判非军事策略 | 军事占比越高 → 科研投入越低（同一条分配公式的自然结果）；不额外为"预判"写逻辑 | `WP-6.3` |

### 10.7.2 必须先问用户的口径（不阻塞开工，先用缺省并标注）

| # | 待澄清 | 我的缺省（先用它开工，答复后只改配置/常量） | 影响 |
| :--- | :--- | :--- | :--- |
| `P-AI-1` | **"60 年"还是"60 日"**？AI.md 写"初始 60 年内不造兵""无威胁 0%（60 年内）/5%（60 年后）" —— 按 1 年 = 360 日，60 年 = 21600 日（一局可能只有 3600 日），字面读会"整局都不造兵" | **✅ 已定（v0.6.7 / `D93`，用户裁定）**：取 **360 日 = 1 游戏年**（"半年或一年"里的一年）；`design/AI.md` 的 3 处口径（line 38/78/110）同源，落地为 `WP-6.1` 的 `Config/AI.json` → `NoMilitaryDays: 360`（改 180 只需一个数字） | `WP-6.1`、`WP-6.4` 的军队触发 |
| `P-AI-2` | **"AI 性格类型"**没有取值集合（只有"固定科技偏好 = 数学/物理/化学"） | 只实现"科技流派偏好"这一维；性格维度**留空**（配置字段预留但不 consume） | `WP-6.1` 配置 |
| `P-AI-3` | 资源分配公式的"**总资源**"= 存量还是月产出？ | 按**月产出（+ 存量上限内的可用量）**分配：比例是"意向"，实际下单仍受存量约束（否则 AI 会把攒下的钱一次性花光） | `WP-6.3` |
| `P-AI-4` | "保留最低限度的生产与科研资源"**阈值是多少** | 先给"资源存量低于 1 个月维护费 → 停兵/停建"的保守值，集中成配置 | `WP-6.3` |
| `P-AI-5` | 威胁等级的"**核心区 / 领地**"如何定义？ | 出生点半径 R 内 + 已建建筑周围 N 格；判定只看**AI 视野内**的单位（不作弊） | `WP-6.4` |
| `P-AI-6` | AI 是否需要**消灭玩家**才能赢？ | 胜负与人类同一套判据（每月判定 + 全灭，见 §19.3 / `WP-4.19`）—— AI 只是另一个势力 | `WP-6.6` |
| `P-AI-7` | AI 数量、出生点 | 用户已定：**先 1 个 AI**，出生点规则同玩家、离玩家距离随机但足够远 | `WP-6.1`、`WP-5.9` |

> 说明：`P-AI-1`/`P-AI-4` 属**数值手感**（用户介入点 N2），`P-AI-2`/`P-AI-3`/`P-AI-5` 属实现选择（我在批次 6 内自决并登记决策），`P-AI-6` 已由 §20 的胜负口径覆盖。
...
> **记录勘误（v0.6.0）**：F 表（以及 A~I 各表）"判定"列的 ✅ 含义是**设计稿有这条功能**，不等于已实现 —— 实现缺口写在"缺口 & 证据"列（如 F10 判定 ✅ 但缺口是"无事件 `TECH-05`"）。因此 §11 汇总的 **❌ 47 条清单**里出现 F10/F11 与"判定 ✅"并不矛盾；以汇总清单为准。本表为冻结历史，不再改写。
...
> **[已归档 · 由 §19 取代]**（v0.6.0）：本节是 v0.2 的 49 个 WP / 6 批次版本；v0.6.0 重规划为 **66 个 WP / 8 批次**并把状态、验收判据、风险与进度搬进 **§19（活文档）**。本节与 §13.z 只作历史参照，**不再更新**（新增项与状态变更一律进 §19）。
>
...
> **[已归档 · 由 §19 取代]**（v0.6.0）：M0/M1/M2 的判据已改写为 §19.3 的**里程碑判据表**（M0-1/M0-2/M0-3 保留原始口径，M1/M2 按 v0.6.0 重规划收紧）；本节只作历史参照，**不再更新**。
...
> **记录勘误（v0.6.0）**：下面这段表格**不属于 §18.5.4** —— 它是 §18.4.2 缺陷看板在 v0.3.19~v0.3.23 期间的追加行，在 v0.3.23 追加 §18.6 时被误接在本清单之后。内容一字未改，仅在此标明归属（新增条目一律进 §19，本段保持冻结）。
...
- 内容规模：科技 **93/93** 节点 ✓（*v0.9.3 / `WP-7.3`：math 30 · physics 35 · chemistry 28*）· 建筑 4/24 条 · 单位 15/15 ✓（`WP-7.5`） · 事件 20/20 ✓（`WP-7.4`） —— 填表债务（v0.6.0 起归 **§19 批次 7**：`WP-7.3` 科技 93 · `WP-7.2a/b` 建筑 24 · `WP-7.5` 单位 15 · `WP-7.4` 事件 20）
- 资源词汇别名（设计稿 Food / basic minerals → 原型 Gold / Wood，`D60`）—— `RES-*` 填表时一次性重映射

---

# 19. v0.6.0 全流程重规划（活文档）

> **本节是唯一活文档**：里程碑 / 批次-WP 状态 / 验收判据 / 风险 / 无归属项 / 进度总览都在这里维护。
> §0~§18 为**冻结历史**（只允许追加一行 `✅ 已修复（vX / WP-Y.Z）`）；§13、§14 已标 `[已归档 · 由 §19 取代]`。
> **决策只追加**进 §20（D71 起），不在本节改写历史结论。
...
| **用户介入点（只有 4 个 + 2 可选）** | **N1** 视觉与操作手感（M1 后）· **N2** 数值手感 · **N3** AI 行为像不像人（批次 6 交付后）· **N4** 美术与发布（批次 8）；可选：**编辑器专属决策**（如场景摆放）与**提供 BGM 文件** |
...
| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-0.1 | 删无引用文件 + 目录口径统一 | ✅ 完成 | M0-1 |
| WP-0.2 | 无头验收载具（`Tests/SciencePotato.HeadlessChecks`） | ✅ 完成 | M0-1 |
| WP-0.3 | IO / 配置抽象（`IFileSystem` / `IConfigSource` / `IConfigLoader`） | ✅ 完成 | M0-1 |
| WP-0.4 | 地形配置转 JSON（`Config/Terrains.json`） | ✅ 完成 | M0-1 |

### 批次 1 · 时间与装配通电（5 WP · ✅ 全部完成）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-1.1 | `GameClock`（游戏日为唯一单位 / 三档流速 / 暂停 / 逐日派发 / 单帧上限） | ✅ 完成 | M0-1 |
| WP-1.2 | `GodotTimeDriver`（真实秒 → 游戏日的唯一适配点） | ✅ 完成 | M0-1 |
| WP-1.3 | 组合根（`CoreBootstrap` / `CoreServices` / `CoreDependencies` / `GameSession`） | ✅ 完成 | M0-1 |
| WP-1.4 | 7 张配置表装载 + 启动期分级校验（error 阻断 / warning 放行） | ✅ 完成 | M0-1 |
| WP-1.5 | 时间口径重标定（秒 → 游戏日）+ 节拍总线 `GameTimeService` | ✅ 完成 | M0-1 |

### 批次 2 · 单玩家可玩（10 WP · ✅ 全部完成）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-2.1 | 跨树前置（`TECH-07`：物理/化学树可解锁） | ✅ 完成 | M0-2 |
| WP-2.2 | 任务生命周期（真删除 / 范围注销 / 完成即回收） | ✅ 完成 | M0-2 |
| WP-2.3 | 人口增长接线（`PopulationGrowth` 修正器首个消费点） | ✅ 完成 | M0-2 |
| WP-2.4 | 建造者绑定（每建筑同时 1 个） | ✅ 完成 | M0-2 |
| WP-2.5 | 建造链路（资源/地形/科技校验 → 任务 → 完成） | ✅ 完成 | M0-2 |
| WP-2.6 | 建筑升级链路 | ✅ 完成 | M0-2 |
| WP-2.7 | 建筑产出入池（学院 `IdeaGrowth` 等） | ✅ 完成 | M0-2 |
| WP-2.8 | 事件按日掷骰 + 生效期倒计时（`EVT-*`） | ✅ 完成 | M0-2 |
| WP-2.9 | 科技树研究（树内串行 + 并发槽位） | ✅ 完成 | M0-2 |
| WP-2.10 | 领域事件总线（`IDomainEventBus`） | ✅ 完成 | M0-2 |

### 批次 3 · 世界不丢 + 真实移动战斗（10 WP · ✅ 全部完成）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-3.1 | Map 常驻内存（`MapSession`） | ✅ 完成 | M0-3 |
| WP-3.2 | 地图实体持久化（`SaveRebuilder` / `MapSave` v2→v3） | ✅ 完成 | M0-3 |
| WP-3.3 | 统一存档单元（`ISaveStore` / `JsonSaveStore` / 迁移链 / 原子写） | ✅ 完成 | M0-3 |
| WP-3.4 | 占据物与拆除（`PlaceOccupant` / `PlaceBuilding` 唯一入口） | ✅ 完成 | M0-3 |
| WP-3.5 | 移动模型 R1~R7（离散累积 MP + 通行权限 + 寻路） | ✅ 完成 | M0-3 |
| WP-3.6 | 同格交战 + 按日双向结算 + HP 归零阵亡 | ✅ 完成 | M0-3 |
| WP-3.7 | 击杀掉落（`UnitLootService` 消费 `UnitDiedEvent`） | ✅ 完成 | M0-3 |
| WP-3.8 | 敌方封锁（`EnemySpawner` 表驱动 + 封锁格不可建造） | ✅ 完成 | M0-3 |
| WP-3.9 | 月度经济结算（产出/需求/扣减/赤字） | ✅ 完成 | M0-3 |
...
| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-4.1 | 修正器升级（多 target 累加 + 宿主作用域 + 阶段管道） | ✅ 完成（v0.8.3：阶段管道 Building→Tech→Event、多 target 累加、宿主作用域 `GetValueForHost`、同宿主去重） | M2 |
| WP-4.2 | 范围效果（观星台 3 格 / 骨笛工坊 1 格 / 振动与波相邻同类） | ✅ 完成（v0.8.7：`ModifierRange` + 覆盖计数 + `AdjacentSameTypeBonus`；新增观星台/骨笛工坊） | M2 |
| WP-4.3 | 单位标签 + 条件化修正（对近战 +20% / 受伤 −25% / 对建筑 +50%） | ✅ 完成（v0.8.7：`Tags` + `Abilities` 按目标标签现算） | M2 |
| WP-4.4 | 修正作用面扩展（上限 / 时长 / 视野 / 消耗 / 任务速率） | ✅ 完成（v0.8.5：6/6 消费点接线 + 正向断言；视野列随 `WP-4.10`） | M2 |
| WP-4.5 | 产出浮动 ±20% + 观星台改写上下限 | ✅ 完成（v0.8.6：农田 ±20/15/10% 进表 + 月结浮动 + 上下限改写消费点；3 格范围部分归 `WP-4.2`） | M2 |
| WP-4.6 | 驻扎系统（学者/化学家：影响范围、产出加成、宿主消失即失效） | ✅ 完成（v0.8.7：`GarrisonHosts`/`GarrisonModifiers` 按位置现算） | M2 |
| WP-4.7 | 单位合并（同模板、HP 之和 ≤ MaxHP、同格） | ✅ 完成（v0.8.7：同模板 + 相邻/同格 + HP 截断；被并方不推阵亡） | M2 |
| WP-4.8 | **建筑 HP + 夺取**（住房/军事有 HP，归零后可被占） | ✅ 完成（v0.7.1） | M2 |
| WP-4.9 | 区域建筑 + 附属建筑嵌套（`MapCell` 建筑集合 + `HostUId`） | ✅ 完成（v0.8.8：`MapCell.Attachments` + `Building.HostUId` + `PlaceAttachment`/升级不动附属/拆宿主一起清 + 两格无迷雾） | M2 |
...
### 批次 5 · 底座 / UI / 性能（11 WP · **全部完成 ✅**）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| **WP-5.1** | **装配通电**：全部应用服务进组合根 + 逐仓储接线（生产与测试共用一份装配） | ✅ 完成（v0.6.0） | M1 |
| **WP-5.2** | **脚手架修复**：`MapView` 注入 / `CameraController` 接线 + 操作模型（滚轮缩放 / WASD / 左键拖）/ 场景路径大小写 | ✅ 完成（v0.6.0） | M1 |
| **WP-5.3** | 地形外观数据驱动（配置 `Sprite`/`Color`，缺美术用纯色占位）+ `Document/AssetManifest.csv` | ✅ 完成（v0.6.1） | M1 |
| WP-5.4 | 正式表现层（图层化：地形/建筑/单位/迷雾；交互：选址建造、单位指令、事件弹窗） | ✅ 完成（v0.9.8：`MapLayerModel` 图层口径 + `MapInteractionModel` 点击→意图（含事件决策与门控查询）+ `MapView`/`MapCellView` 最小接线（图层容器、雾 `Modulate`、占据物占位六边形、世界坐标→格位逆运算）；6 条无头用例） | M2 |
| WP-5.5 | 迷雾性能（半径模板缓存 + 增量更新 + 紧凑存档） | ✅ 完成（v0.9.7：`FogGeometry` 模板缓存（构建次数与调用解耦）+ `FogCodec` 紧凑编码（zigzag varint + Base64，~3 字节/格）+ 脏格增量与跳过写盘 + 老 `MatrixData` 兼容；6 条用例） | M2 |
| WP-5.6 | 地图存档性能（脏格子增量写 + `DeleteMap`/`ListMaps` 补全） | ✅ 完成（v0.9.7：`MapSaveMerger` 增量合并 + 两仓统一脏格统计与"无变化跳过写盘" + 应用层 `DeleteMap`/`ListMaps`（`MAP-07`/`MAP-08`）；6 条用例） | M2 |
...
| WP-6.4 | AI 军事（60 日不造兵 → 玩家暴露后 10~20% 资源造兵 → 威胁分级 → 优先防御） | ✅ 完成（v0.7.5，窗口 = 360 日；军费分级 5/10/15/20%，训练走 `TrainUnit`、守家走 `CanMove`） | M2 |
...
| WP-7.2b | 其余建筑 + 3 个独立建筑（24 条全表 + 校验 0 error） | ✅ 完成（v0.8.8：24 条全表达成——观星台/骨笛工坊/日晷进表，引用校验 0 error；数值复核归 N2/`WP-7.6`） | M2 |
...
### 批次 8 · 美术与音频（4 WP · 完成 1 / 待素材 2 / 未开始 1）

| WP | 内容 | 状态 | M |
| :--- | :--- | :--- | :--- |
| WP-8.1 | 美术规格 + `AssetManifest` 定稿（尺寸/锚点/命名/授权来源） | ✅ 完成（v0.9.10：`ArtSpec.md` v0.9.10（七条硬规则 + 189 张尺寸总表 + UI 28 + 授权/署名 + 分包 + 验收 + FAQ）· `Tools/gen_asset_manifest.ps1` 从四表生成 **189 行** `AssetManifest.csv` + `AssetList.md` · `Tools/check_assets.ps1` 自动验收（尺寸/alpha/命名/缺图/多余文件 + `-Promote`）· `Credits.md`；**修正**：地形图幅 352×406 → **366×423**） | M3 |
| WP-8.2 | 资源替换（按 Manifest 换图/换音，**不改代码** —— 由 `WP-5.3` 保证） | 🟡 接线已完成、等美术文件（v0.9.10：`MapLayerModel` 路径口径 + `MapCellView` 有图用图/无图回退占位/自动缩放/脚底对齐；`PresentationChecks` 锁住路径与清单闭合 ⇒ 美术丢文件即生效，`git diff` 里不会有 `.cs`） | M3 |
| WP-8.3 | 双语完工（文案全量 + 术语表） | ☐ 未开始 | M3 |
| WP-8.4 | BGM + 打包（导出模板 + `*.json`/资源清单纳入导出） | 🟡 音频契约/清单就绪（v0.9.11：`SoundCatalog` + `SoundChecks` 5 条 + `SoundSpec`/`SoundList`/`SoundManifest` + `gen_sound_list.ps1` + `check_assets.ps1` 覆盖图音）；播放器 `AudioAppService` 与打包等音频/素材到位 | M3 |


## 19.3 里程碑判据（逐条可自动验证）

| 里程碑 | 组成 | 判据（**可自动验证**的部分） | 需要人看的部分 |
| :--- | :--- | :--- | :--- |
...
| # | 风险 | 为什么是真风险 | 现在采取的对策 | 归属 |
| :--- | :--- | :--- | :--- | :--- |
| `R1` | **73×143 = 10439 格**：存档体积、迷雾矩阵、渲染节点数 | **已建基线（`WP-5.8` / `Document/PerfBaseline.md`）**：生成 218 ms · 90 日推进 31 ms · 存档 34 ms · 读档 88 ms · 渲染 **933 ms** · 地图文件 **2.4 MB** · 静态内存 **86 MB**（10k 节点 ≈ 6 KB/格）。迷雾尚未在这个尺度上跑过 | ① 冒烟固定 10439 格 + `--size=WxH` 可换规模；② 耗时/内存护栏（宽松，只抓数量级退化）；③ **地图存档 2.4 MB 是磁盘大头 → `WP-5.6` 优先级提高**；④ 渲染在 `WP-5.4` 加图层前必须定"图块合批 / 视口裁剪" | `WP-5.8` ✅、`WP-5.5`、`WP-5.6`、`WP-5.4` |
| `R2` | **AI 在 1 万格上的决策/寻路开销** | AI 若每 tick 全图扫描，1 个 AI 也能把帧率打下来 | ① 决策 tick 按**游戏日**而非每帧（`WP-6.2`）；② 只读自己的视野（天然裁剪）；③ 目标是"像人"而不是"最优" | `WP-6.2`、`WP-6.5` |
| `R3` | **内容体量**（93 + 24 + 20 + 15 条全表） | 填表量远大于代码量，且数值必须自洽 | ① 全表归批次 7，且**逐表校验**（`ConfigValidator` 已能抓引用错误）；② `WP-7.6` 做整表一致性 + 抽样长跑 | 批次 7 |
| `R4` | **i18n 越晚越贵** | 字符串一旦散进 UI 代码，回头抽取等于重写 UI | `WP-5.10` 排在 M1（在正式 UI `WP-5.4` **之前**）建框架，之后所有 UI 必须走键 | `WP-5.10` → `WP-5.4` |
| `R5` | **"换美术不改代码"是硬要求** | 这条决定了美术能否在 M3 由别人并行推进 | ① `WP-5.3` 外观数据驱动 + `Document/AssetManifest.csv`（美术只改文件与清单）；② 缺图不崩、有兜底（已实现：缺贴图只警告并占位） | `WP-5.3`、`WP-8.2` |
| `R6` | **胜负依赖链**：要判胜负，先要有建筑 HP/夺取、AI、月度判定 | 胜负是"很多 WP 的汇聚点"，容易被当成一个小功能排到最后 | ① 口径先定（`D72`：每月判定 + 全灭）；② `WP-4.19` 明确依赖 `4.8`/`4.17`/`6.6` 并在 §19.2 标出 | `WP-4.19`、`WP-4.8`、`WP-6.6` |
| `R7` | **事件仅对玩家**（AI 不被打断） | 若照抄"每个势力都跑事件"，AI 会被决策暂停打断（设计稿明确禁止） | 已实现并用例锁住：`SessionOrchestrator` 只给人类启动事件引擎；AI 侧 `IsEngineStarted == false` | `WP-4.18` ✅ / `WP-6.5` 复核 |

## 19.5 无归属 / 待澄清（有答复就改这一张表）

| # | 项 | 现状 / 缺省 | 谁来定 | 归属 |
| :--- | :--- | :--- | :--- | :--- |
| `U1` | **"夺取"细节**：HP > 0 的敌方建筑格不可进入（先打）→ HP 归零转为"可夺取"→ 我方单位站上即易主（HP 归零**不**自动易主） | 已按此实现为口径（`D73`），等用户一句确认 | 用户（一句话） | `WP-4.8` |
| `U2` | **73×143 的行列方向**（73 是宽还是高） | `WP-5.8` 已实测：**两种方向的耗时/内存/存档体积几乎一致**（生成 214 vs 218 ms、渲染 958 vs 933 ms、地图文件都是 2.4 MB）—— 也就是说这是**纯观感问题**（哪个方向少滚屏），不是性能问题。**✅ 已定（v0.6.7 / `D92`，用户裁定）**：**横幅 = 143 列 × 73 行**（世界 52338×23141，贴合 16:9，少滚一半）。默认值仍只在一处（`DevMapUi.DefaultWidth/DefaultHeight`）。 | ✅ 已关闭 | `WP-5.4`（正式 HUD 会显示尺度） |
| `U3` | `P-AI-1`（"60 年"还是"60 日"）· `P-AI-4`（最低保留阈值） | 见 §10.7.2 的缺省值 | 用户（数值手感 N2） | `WP-6.3`/`WP-6.4` |
| `U4` | `P-AI-2`/`P-AI-3`/`P-AI-5`（AI 性格枚举 / 分配基数 / 领地定义） | 我在批次 6 内自决并登记 §20 | 我 | `WP-6.1`~`WP-6.4` |
...
| `U9` | **建筑 HP 数值待定**（设计稿只写"住房与军事建筑有 HP、可被夺取"，没给数值）：当前取 营地 lv.I/II/III = **200/300/400**、军营 lv.I/II/III = **250/400/600** | 机制已就位、填表即生效；数值由人裁量（**N2** 数值手感时一起看），`WP-7.6` 平衡校验复核 | 用户（N2） | `WP-7.6` |
| `U10` | **夺取后视野不随建筑迁移**：`FogAppService` 目前是单 owner 实例（迷雾按 owner 拆分归 `WP-4.10`），所以被夺走的房子不会把"谁看得见"一起换过去 | 修正器已随夺取移交（`WP-4.8`），视野要等 `WP-4.10` 的按 owner 迷雾就位后一并处理 | 我（实现） | `WP-4.10` |
| `U8` | **开局经济死锁（数值，需裁量）**：初始 `BasicMinerals` **200**（`resources.md` 初始储备）< 最便宜的**产出**建筑（农田/矿场）**600**（`buildings.md` 建造消耗）⇒ 按字面数值开局造不起任何产出来源，石材永不增长 | **✅ 已定（v0.6.7 / `D91`，用户裁定方案 a）**：初始 `BasicMinerals` 提到 **800**（≥ 最便宜产出建筑 600 ⇒ 开局能建 1 座矿场还剩 200，不再死锁）。用例已把"初始 ≥ 最便宜产出建筑"钉成断言。**遗留**：营地/工坊/学院/军营造价仍是原型遗留值，`WP-7.2b` 按设计稿重填后需重跑一次该断言（若届时不成立，我会带着数字来问） | ✅ 已关闭 | `WP-7.2b` / `WP-7.6` |

## 19.6 进度总览与复现命令

**进度**：批次 0~3 ✅（29 WP）· 批次 4 **19/19 ✅** · 批次 5 **11/11 ✅** · 批次 6 **6/6 ✅** · 批次 7 **7/7 ✅** · 批次 8 **1/4 + 2 就绪**（`8.1` ✅ · `8.2` 等美术文件 · `8.4` 音频契约清单就绪等音频 · `8.3` 未开始）→ **已完成 73 / 剩余 3**（**M2 全部完成 ✅**；剩 `WP-8.2` 收图 + `WP-8.3` 双语完工 + `WP-8.4` BGM/打包）。
...
# ② 无头验收（纯 C# 核心；退出码 0 = 全部通过）
dotnet run --project Tests\SciencePotato.HeadlessChecks

# ③ Godot 无头冒烟（真引擎：装配 / 资源加载 / 73×143 / user:// 落盘）
#    `--smoke` 让 ServiceContainer 用独立的 user://save/smoke.json，不碰正式档
...
#    退出码 0 且 stdout 里 `[SMOKE] 汇总：通过 11 / 失败 0`
```

> 说明：③ 是多 AI 势力的验收入口 —— 加 `--ai=1` 即可让本局多一个 AI 势力（先只有"资源 + 月结"，决策在批次 6）。

---

# 20. 决策记录（只追加，D71 起）

> 编号连续、**只追加不改写**（要推翻旧决策就追加一条"取代 `Dxx`"）。每条 = 日期 / 决策 / 为什么 / 影响面。
> v0.3 之前的决策（`D1`~`D70`）散落在 §18.3 与各版本变更记录中，保持原样不动。

| # | 日期 | 决策 | 为什么 | 影响面 |
| :--- | :--- | :--- | :--- | :--- |
| `D71` | 2026-09-15 | **关闭 Q1 / Q5 / Q7**：① 多玩家形态 = **单机 + N 个 AI 对手**（不做热座/联机）；② **不做**对象级 buff 宿主化（修正器继续按 `(mapId, ownerId, target)` 聚合，靠注销保证"宿主没了加成也没了"）；③ 存档**允许弃档**但**保留迁移链**（破坏性改动升 `SaveVersion`，新档必须能读到最新） | ① 联机需要确定性模拟/权威服务器/回放，与"到 M2 = 全内容 + AI"的目标相比性价比极低；② 现有修正器模型已够表达设计稿全部效果，宿主化是纯重构；③ 原型期"为了兼容而兼容"会拖慢每次结构演进 | 路线图（去掉联机相关项）、`WP-4.18`/批次 6、`MOD-03` 降级、`DEP-06` 口径明确 |
| `D72` | 2026-09-15 | **胜负口径** = **每月判定**（月结时评估是否有人出局）+ **全灭**（所有单位与建筑都没了立即出局）；AI 与人类**同一套判据** | 胜负必须可自动验证且不依赖额外子系统；月结已有稳定节拍（`WP-3.9`），全灭是最无争议的即时判据 | `WP-4.19`（实现）、`WP-6.6`（AI 侧）、`WP-5.11`（结算界面） |
| `D73` | 2026-09-15 | **夺取口径** = 敌方建筑 HP > 0 时该格**不可进入**（先打）；HP 归零转为"可夺取"；**我方单位站上该格即易主**（HP 归零**不**自动易主） | "打掉血"与"占领"分开，玩家才有"补刀/抢点"的操作空间；自动易主会让远程单位隔空夺城 | `WP-4.8`（建筑 HP + 夺取）、`WP-4.9`（格内多占据物时的判定）、AI 军事（`WP-6.4`） |
| `D74` | 2026-09-15 | **装配通电口径**：换装点**只有** `CoreBootstrap` 一处（宿主与用例共用）；**没有注入 `ISaveStore` 时不接线任何持久化仓储**（保持"无存档单元 = 无持久化"） | ① 改造前的病根是"这个服务只存在于测试夹具里"，唯一解是让两边走同一个入口；② 若在无存档单元时仍建仓储，测试会往 `user://...` 这种伪路径写盘，把契约弄脏 | `WP-5.1`（已实现）、所有后续新服务（**必须**在组合根注册，并为 `CoreServices` 增加只读属性） |
| `D75` | 2026-09-15 | **事件引擎只对"人类玩家"启动**；AI 势力不跑事件引擎 | `design/AI.md` 明确"AI 不会面对任何事件"；否则 AI 会被决策暂停打断（或被迫替玩家做选择） | `WP-4.18`（已实现 + 用例锁住）、`WP-4.12`（暂停决策只针对人类）、`WP-6.5` |
| `D76` | 2026-09-15 | **缺省地图规模 = 73×143（10439 格）**，开发面板与冒烟都用它；行列方向按字面 `宽×高` 传入，由 `WP-5.8` 复核后定死 | 玩具图（5×5）会把规模问题（存档/迷雾/渲染/AI 开销）藏到 M2 之后；早暴露便宜得多 | `DevMapUi`、冒烟、`WP-5.8`、`WP-6.2`（AI 开销） |
| `D77` | 2026-09-15 | **Godot 无头冒烟成为标准验收环节**：入口 `Scene/Dev/smoke_report.tscn`，用户参数 `--smoke`（独立 `user://save/smoke.json` + 每次运行先删旧档保证可复现），退出码即结论 | 纯 C# 用例永远碰不到引擎：资源加载、autoload 顺序、场景脚本注入、`user://` 落盘只有真引擎跑得出来 | 所有后续 WP（交付前跑三条命令）、`WP-8.4`（导出后同样跑） |
| `D78` | 2026-09-15 | **ownerId 约定**：`>= 1` = 势力（人类或 AI，有资源池/科技/迷雾/建筑/单位）；`0` = 中立；`< 0` = 野怪（敌方封锁单位，不参与胜负）。玩家表缺省 = 单人类玩家（`owner=1`，保持既有语义） | 把"谁是玩家"从隐式约定变成显式数据，是"加 AI 不串味"的前提；同时不惊扰既有 170 条用例 | `WP-4.18`（已实现）、批次 6、`WP-4.19`（谁参与胜负）、`WP-4.10`（迷雾按 owner） |
| `D79` | 2026-09-15 | **文档纪律**：`log.md` 分三类 —— **§19 活文档**（里程碑/批次-WP 状态/验收判据/风险/无归属/进度）、**§20 决策记录**（只追加）、§0~§18 冻结历史（只允许追加一行 `✅ 已修复（vX / WP-Y.Z）`）；§13/§14 标 `[已归档 · 由 §19 取代]`；每完成一个 WP 只改三处（§19.2 状态、§19.6 进度、§20 决策） | 旧写法是"每批把新总结接在旧总结后面"，读的人要先分辨哪段还有效；三类分区让"当前有效信息"永远只有一处 | 全文、后续所有批次 |
| `D80` | 2026-09-15 | **`SessionOrchestrator.StartMap` 幂等**：同一 `(mapId, ownerId)` 重复调用不重复登记任务；地图不存在时**什么都不启动**（不创建孤儿存档分区），原因写入 `LastStartSkippedReason` | "开局 / 生成地图 / 读档后"三个入口都可能调用它，若不做幂等，月结会翻倍；若地图不存在还启动，存档里会留下没人认领的分区 | `WP-4.18`（已实现 + 用例锁住）、`WP-5.11`（读档流程）、`WP-5.9`（开局流程） |
...
| `D91` | 2026-09-15 | **初始基础石材 = 800（用户裁定方案 a）**：`Config/Resources.json` 的 `BasicMinerals.BaseValue` 由设计稿的 200 提到 800 —— 让"开局能造起第一个产出来源"成立（最便宜产出建筑 600），否则石材永不增长（经济死锁） | 设计稿的"初始储备 200"与"农田/矿场造价 600"按字面组合会让开局无解；数值手感由用户裁量（`N2`），我给出两个候选后用户选 a。用例把"初始 ≥ 最便宜产出建筑"钉成断言（`ContentBuildingChecks.StartingStockAllowsAProducer`） | `Config/Resources.json`、`ConfigTableChecks`（初始储备断言 800）、`ConfigTableGuide.txt` v1.11、§19.5 `U8` |
| `D92` | 2026-09-15 | **地图方向 = 横幅 143 列 × 73 行（用户裁定）**：`DevMapUi.DefaultWidth/DefaultHeight = 143/73`（10439 格不变），世界尺寸 52338×23141，贴合 16:9 | `WP-5.8` 实测两种方向的耗时/内存/存档体积几乎一致 ⇒ 这是纯观感问题：竖幅在 16:9 屏上要来回滚，横幅少滚一半。默认值只在一处，换向只改两个常量 | `DevMapUi`、`Document/PerfBaseline.md`（基线改按 143×73 记）、`Document/ArtSpec.md`、§19.5 `U2` |
| `D93` | 2026-09-15 | **AI 前期不造兵窗口 = 360 日（用户裁定"半年或一年"，取一年）**：`design/AI.md` 的三处"60 年"（line 38/78/110）统一解释为 **1 游戏年 = 360 日**，落地为 `WP-6.1` 的 `Config/AI.json` → `NoMilitaryDays: 360` | 按字面 60 年 = 21600 日会让 AI 整局不造兵（一局常见规模约 3600 日）；360 日在标准流速下 ≈ 6 分钟真实时间，既"侧重发展"又能在玩家扩张后开始自卫 | `WP-6.1` 配置、`WP-6.4` 军队触发、§10.7.2 `P-AI-1`、§19.5 `U3` |
| `D94` | 2026-09-15 | **执行工作流（用户要求提速）**：① 三层验收收进 `Tools/verify.ps1`（构建 → 无头检查 **4 路并行分片** → 冒烟，结论一行，日志写 `%TEMP%`）；② 分片用 `--only-group=` **精确**匹配（避免 "Config" 撞 "ConfigTable"）；③ 新增 `Document/CodeMap.md`（入口/依赖/配置链路/用例组的"改哪里要配套改哪里"）；④ 迭代只用"build → 直跑 exe"（**禁用 `dotnet run`**：无 `--no-build` 时全在等 MSBuild）；⑤ 编辑纪律：`old_text` 不带行尾换行、插值串内不写引号/单引号、机械改名用 `.Replace()` 且改完立刻 grep 验证；⑥ 口径类文档（TestPlan/Guide/ArtSpec/PerfBaseline）改为**每批末**统一更新，状态类（§19.2/§19.6/§20）仍逐 WP | 实测：全套无头 22.2 s、测试工程 build 15.7 s、`dotnet run` 让"跑一组"涨到 15 s+；再叠加命令超时与手误重试，一次提交前的验证要 40~60 s、跨 3~6 次调用。并行脚本把它压到 **16.6 s / 1 次命令**，且不减少任何一条断言（`-Serial` 可退回串行排查） | `Tools/verify.ps1`、`Tests/…/Program.cs`（`--list-groups` / `--only-group=`）、`Document/CodeMap.md`、`Document/TestPlan.md` |
| `D95` | 2026-09-15 | **胜负判定的实现口径（`WP-4.19`）**：① **参与者 = 玩家表**（中立/野怪不在表里 ⇒ 天然不参与胜负）；② **出局判据 = 既无单位也无建筑**（交战中的进攻方也算占据物，`GetOccupants` 已含 `Invader`）；③ **判定触发点只有两处**：`UnitDiedEvent`（全灭）+ 月结推送（每月判定）；建筑被毁在 `WP-4.8` 里接同一个 `Evaluate` 调用；④ 只剩一个存活者 ⇒ 胜者，零个存活 ⇒ 同归于尽（胜者为空）；⑤ **幂等**：已出局的势力不再改状态、已结束的对局不再改结果；⑥ `VictoryService` 只认 `ownerId`，不认识"谁是 AI"（`D72`/`D78`：人类与 AI 同一套判据） | 逐帧扫描 10439 格找"谁没了"是纯浪费（`R1`）；而全灭只可能由"单位阵亡/建筑被毁"造成，两者都有推送 —— 用推送 + 月结兜底既准确又便宜。"参与者取自玩家表"还顺手解决了"野怪算不算敌人"这个歧义（不算） | `Scripts/Core/VictoryService.cs`、`CoreServices.Victory`、`CoreBootstrap` 6.9、用例 `VictoryChecks`（5 条）、`Document/CodeMap.md` |
| `D96` | 2026-09-15 | **无存档单元时的仓储路径缺陷（`WP-5.1` 的漏网）**：装配层只在 store 非空时才给**任务/时钟**仓储接线，但**资源/科技/迷雾/修正器**仓储仍按 `SaveRoot`（缺省 `user://save/`）建路径 —— 无存档单元的夹具一旦走到写盘（建资源池就是第一次），Windows 会把 `user:\save\...` 当非法路径抛异常。**修法**：`store == null` 时把仓储前缀整体改到系统临时目录（语义仍是"不持久化"，且不污染仓库），并在 `WiringChecks` 里加回归锁（资源池可读写 + 仓库根目录不长出 `user:`） | 这是"约定只写进注释、没落进装配"的典型：`D74` 说了"无存档单元 = 无持久化"，实现只覆盖了一半的仓储。它在 `WP-4.19` 的用例第一次跑到"建资源池"时炸出来 —— 早发现是因为新用例走的是**生产装配**（`D81`） | `CoreBootstrap` 3.3、`WiringChecks.NoStoreKeepsMemoryOnly`、§19.5 无新增（已当场修） |
| `D97` | 2026-09-15 | **建筑 HP 与夺取的实现口径（`WP-4.8`）**：① 新增 `IDamageable`（Common/Domain，`Map` 只认接口，不认 `Building`）；② **HP 归零 = 从占据物槽位摘下**（格子因此可进入），但建筑仍在 `cell.Building` + `_occupants` 索引里 —— 它还是原主人的资产（`D73` 的"归零不自动易主"）；③ **夺取 = 单位进入该格**，发生在 `Map.PlaceOccupant/MoveOccupant` 的落位之后（唯一入口），归属改写 + HP 恢复半血；④ `MapAppService.GetOccupants` 必须计入 `cell.Building`（漏掉会让"守着零血房子的人"被判全灭、建筑维护费凭空消失）；⑤ 伤害的推送链：`UnitCombatService` → `MapAppService.ApplyBuildingDamage` → `Map.ApplyBuildingDamage`；⑥ **建筑资产变化接入胜负重算**（`BuildingRemovedEvent`/`BuildingCapturedEvent` → `VictoryService.Evaluate`） | 一格一占据物的模型不能同时容纳"建筑站着"与"单位站上来"——"HP 归零摘槽位"是**最小改动**且完全符合设计稿的夺取动作；而"夺取发生在落位之后"让它只出现在一个地方（所有进入路径共用，含训练完成落位）。⑤/⑥ 是"资产变化"这件事在推送侧的唯一出口（`D95` ③ 的实现） | `Scripts/Common/Domain/IDamageable.cs`、`Building`（HP/`CaptureBy`/`BuildInfo`）、`BuildingConfigDto.HasHP/HP`、`Map.ApplyBuildingDamage`/`TryCaptureAt`、`MapAppService`（伤害入口 + `GetOccupants` 计入建筑 + 两条推送）、`UnitCombatService`、`VictoryService`、用例 `BuildingHpChecks`（5 条）、§19.5 `U9`/`U10` |
...
| `D103` | 2026-09-15 | **AI 军事口径（`WP-6.4`）**：① 新增 `AiMilitaryPolicy`（**唯一出处**，`AiService` 的"计划军费"与 `AiMilitaryService` 的"真花"都调它）：窗口内（`day < NoMilitaryDays`）**0%** / 窗口后无威胁 **5%**（设计稿"60 年后"的基础守备）/ 低 **10%** / 中 **15%** / 高·致命 **20%**（设计稿"玩家暴露后 10~20%"）；**生存优先时一律 0%**；② **预算 = 当前石材 × 军费占比**，预算买不起一个兵就不造（不把经济吃空）；候选 = 军营 `TrainableUnits` 里预算内的**最便宜**者，训练一律走 `UnitsAppService.TrainUnit`（军营/队列上限/人口/资源全由它校验）；③ **优先防御** = 威胁 ≥ High 时把 `Actions` 含 `CanAttack` 的自家单位叫回**最近自家建筑**（已在家门口的不动，避免每拍重发指令刷掉状态），移动走 `ExcuteAction(..., "CanMove")`；④ **不主动侵略**（`A-AI-9`）：本 WP 只做"守家 + 按预算造兵"，派兵拆家留到 `N3` 之后 | ① 一份策略两处引用，杜绝"决策说 20%、实际花 5%"这类口径漂移；② 预算化比"有钱就造"更像人，也顺带满足"不在资源低时暴兵"的验收行；③ 守家是最小可验的"优先防御"，且完全复用玩家移动链路（能力门控/MP/寻路都不重写） | `Scripts/AI/Domain/AiMilitaryPolicy.cs`、`AiMilitaryResult.cs`、`Scripts/AI/Application/AiMilitaryService.cs`、`AiService.Decide`（军费改用 policy）、`AiService`（支持多行动出口）、`CoreBootstrap`（6.12）、用例 `AiMilitaryChecks`（5 条） |
| `D104` | 2026-09-15 | **"不作弊"钉成差分判据（`WP-6.5`）**：不看"AI 做了没有"，而是**改变 AI 不该知道的东西、断言它的判断/行为不变**：① 视野外的敌人 ≡ 世上没有这个敌人（并带**对照组**：同一敌人进视野后指纹必须变，防判据恒真）；② 对手资产/资源多寡不影响 AI（不读对手账本）；③ 无军营时跑多拍不涨兵、新建建筑只能长在自家单位 7 格内（不凭空生成）；④ 资源不超配置上限（没有隐藏收入）；⑤ 玩家在研究什么都改变不了 AI 选哪个节点（不完美克制） | 这类缺陷的表现是"AI 悄悄变聪明"，正向断言（"AI 造了兵"）永远抓不到；差分判据把"信息边界"变成可执行规格，且**不需要**为它写生产代码（判据即护栏） | `Tests/SciencePotato.HeadlessChecks/AiFairnessChecks.cs`（5 条）、§19.4 的 `A-AI-2`/`A-AI-9` |
| `D105` | 2026-09-15 | **AI 与胜负口径（`WP-6.6`）**：① **出局即停** —— `AiService` 挂 `VictoryService` 后，`Evaluate` 对"既无单位也无建筑"的势力返回 `null`（不记录、不下单），并计一次 `EliminatedSkips`；`StartEngine` 的 tick 回调发现返回 `null` 就 **`Unregister` 自己** ⇒ 时间轴上不留僵尸 AI；② 胜负**双方同构**：AI 全灭 → 人类胜、人类全灭 → AI 胜（`VictoryService` 的参与者就是玩家表，无需为 AI 特判）；③ 一方出局不影响另一方继续决策 | ① "AI 死了还在研究/造兵"是 M2 最容易漏的体验缺陷（资产事件与决策引擎是两条独立链路）；② 自摘 tick 比"每拍判断一次再跳过"更省也更干净（订阅数回落可直接断言） | `AiService.AttachVictory`/`Evaluate`/`StartEngine`、`CoreBootstrap`（6.13）、用例 `AiVictoryChecks`（5 条）、`SmokeReport`（AI 经济/军事实测行 + "窗口内不造兵"判据） |
| `D106` | 2026-09-16 | **事件触发即暂停的口径（`WP-4.12`，`M2` 判据 ③）**：① `EventAppService` 在事件命中后把它挂进**待决队列**（`PendingEventDecision`：mapId/ownerId/eventId/name/day/duration）并**自动暂停**时间轴；② 暂停 = 把 `GameClock.Speed` 设为 `Paused`，但**先记住玩家原来的档位**，`Resolve`/`ResolveAll` 清空该势力的待决队列时**还原原档位**（不硬编码回 Standard，也不吃掉玩家的选择）；③ 多待决必须**全部**确认才恢复（同一日多个事件命中会堆起来）；④ **暂停只作用于真实时间推进**（`GameClock.Advance(realSeconds)` 返回 0 日）；`AdvanceDays` 是无头模拟/离线结算的推进原语，**不受暂停影响**（否则既有 `EventEngineChecks` 的长跑与离线结算会全被冻住，且测试无法用同一入口跳日）；⑤ 玩家自己已暂停时不再记档位、不覆盖（`AutoPauseCount` 只在真暂停时 +1）；⑥ 只有**跑事件引擎**的势力（人类，`D75`）会有待决 ⇒ AI 侧天然不被暂停 | ① "触发即暂停"的价值在于**玩家有时间重新规划**，所以恢复必须回到他原来的档位；② 待决队列 + `Resolve`/`ResolveAll` 是纯 C# 契约，UI（`WP-5.4` 事件弹窗 / `WP-5.11` 会话入口）只需调它，不必碰时间系统；③ 不改 `AdvanceDays` 语义是关键取舍：无头验收要能跳日，暂停只该拦住"真实时间流逝" | `Scripts/Events/Domain/PendingEventDecision.cs`、`Scripts/Events/Application/EventAppService.cs`（待决队列 + 自动暂停/恢复）、`EventAppService` ctor 新增 `clock`、`CoreBootstrap`（传 clock）、用例 `EventPauseChecks`（5 条） |
| `D107` | 2026-09-16 | **单位全表（15 条）口径（`WP-7.5`）**：① 玩家 10 + 敌方 5，与 `design/unit.md` 一一对应；**敌方 5 条在改造前就已与设计稿一致**（地形/概率/掉落/原地封锁），只补了玩家侧 7 条（`explorer`/`spearman`/`heavy_guard`/`engineer`/`ballista`/`scholar`/`chemist`）；② **`swordsman` 这个 id 保留给设计稿的"民兵"**（不重命名：`swordsman` 已被战斗/训练/AI 用例引用几十处，重命名是纯噪声；改名归 `WP-7.6` 统一做，或永久保留并把 i18n 名改成"民兵"）；③ **训练来源按设计稿分派**：工坊 lv.I=工人 / lv.II=+工程师 / lv.III=+化学家；军营 lv.I=民兵+探险者 / lv.II=+弓箭手+长矛兵 / lv.III=+重装卫士+弩炮；学院 lv.II=学者（并给它补上 `CanTrain`）；④ **数值沿用现值、不按设计稿改 HP/伤害**：战斗平衡属 `N2`（用户裁量），而"全表齐备"是 `M2` 判据 ② —— 两件事分开做，避免用"填表"顺手改掉已被 20+ 条用例钉住的战斗数值；⑤ 只做了两处**必要**的配置修正：`engineer` 补 `CanAttack`（设计稿给它攻击力 5，而校验器规定"有伤害必须能主动开战"）、`school_ii` 补 `CanTrain`（否则学者无训练入口）；⑥ 新增 `ContentCoverageChecks`（3 条）：数量与设计稿一致、**每个玩家单位都能在某建筑 `TrainableUnits` 里查到**（内容自洽判据）、敌方三件套齐备且玩家单位不带生成字段 | ①②④ 都是"改动最小、判据最强"的取舍：用判据锁住设计稿意图，把纯改名与数值平衡从"填表"里剥出去；③⑤ 是"填表必须同时接上链路"的实例——只往表里塞单位而不派训练来源，游戏里就是造不出来的死内容；⑥ 训练来源闭合是**填表期最值得自动查的一条**（人眼几乎不可能发现"学者没有入口"） | `Config/Units.json`（15 条全表）、`Config/Buildings.json`（`TrainableUnits` 分派 + `school_ii.Actions`）、`Tests/.../ContentCoverageChecks.cs`（新）、`ConfigTableChecks`/`EnemySpawnChecks`/`TrainingQueueChecks` 三处旧计数口径（8→15、3→10、工坊/军营名单） |
| `D108` | 2026-09-16 | **事件全表（20 条）口径（`WP-7.4`）**：① 20 条全部落在 `Config/Events.json`，概率/持续期/消耗/修正器按 `design/events.md` 逐条落地（"永久" = `Duration: 0`，因为校验器规定负数是 error）；② **修正器目标名一律取 `Canonical`**（`MineralGrowth`/`FoodGrowth`/`IdeaGrowth`/`PopulationGrowth`/`BuildingSpeed`）——"每月 +N"按 `Absolute`、"±X%"按 `Percent`（与建筑表同口径：农田 144/年 = 12/月）；③ **科技前置暂空**：设计稿的 17 条前置（度量/基础几何/记数系统/毕达哥拉斯学派/几何原本/影子观测/制陶术/炼金术兴起/镜面反射）指向的节点**在 `WP-7.3` 才存在**，而 `ConfigValidator` 把"未知节点"记 warning、`TechPrerequisiteChecks` 又要求"真实配置只允许 Duration=0 一类 warning"——所以先留空，`WP-7.6` 在科技表落地后**统一回填**（已登记）；④ 3 处旧计数口径（事件 3 条）随全表更新 | ① 全表落地是本轮 `M2` 判据 ② 的第三块（单位 15 ✓ / 事件 20 ✓ / 建筑与科技待做）；③ 是本轮最值得记的一条**依赖顺序**教训：内容表之间有引用（事件→科技、建筑→科技），填表必须按"被引用者优先"排序，否则要么带 warning 硬塞、要么写完再返工 | `Config/Events.json`（20 条全表）、`ConfigTableChecks`（2 处）、`EventEngineChecks`（1 处）计数口径 |
| `D109` | 2026-09-16 | **科技表（93 节点）的落地基础设施（`WP-7.3` 第一段，表本体下一轮落地）**：① 新增 **`Tools/gen_tech.ps1`**：直接解析 `design/research_tree.md` 的定宽表生成 `Config/TechTrees.json`（零手工转录 ⇒ 不会漏行/抄错），实测解析 **93 行**（science 30 / physics 35 / chemistry 28）、**64 条能映射到当前机制**（产出/上限/视野/建造与升级折扣/人口/消耗/单位属性）、**29 条只留 EffectText**（范围效果 `WP-4.2`、地形通行 `WP-4.11`、UI 门控 `WP-4.14`、"解锁 X 建筑"由建筑表承载）；② 节点 DTO/接口新增 **`Name`（设计稿中文名）+ `EffectText`（效果原文）**：93 条效果里有一部分当前没有机制可落，**原文留档**才不会"悄悄丢掉一条设计意图"；③ `ModifierTargetRegistry` 新增两个目标名 `BuildingUpgradeCost` / `FoodConsumption`（设计稿确有"升级消耗 -10%""食物消耗 -10%"这类效果）；④ **表本体与用例迁移一并延后**：老用例编码的是**占位树形状**（`science` 树有两个根 `writing`/`counting`、`mathematics ← writing` 的单链、以及设计稿里根本没有的 `military` 树），一次性替换会打掉 12 条断言。**已定的迁移方案（下一轮照做）**：不去改设计表去迁就用例，而是让**用例夹具注入 test-only 根节点**（在该用例的 `TechTrees` 夹具副本里往 science/physics 各加一个 `fixture_root`），从而"同树两个根 / 两树并行 / 并发可配 / 任务键"四条并发语义**原样保留**；其余引用改成设计树真节点（`science:counting`、`physics:simple_machine_intuition`、`science:mathematics` 三个遗留 Id 继续沿用，见 ①） | ① 生成器让"填 93 条"从抄写变成可重复执行的转换（改设计稿 → 重跑脚本），这是本 WP 花得最值的一笔；④ 是本轮最贵的教训：**替换一张被 12 条用例钉住的表之前，先看清那些用例到底在测什么** —— 它们测的是"槽位/前置/任务键"机制，并不依赖具体是哪棵树；夹具注入比改内容表更便宜也更安全（本轮已经试过改内容表的代价：11 条失败 → 收敛不动，只能回滚） | `Tools/gen_tech.ps1`（新）、`Scripts/TechTrees/Domain/ITechNodeConfig.cs`、`TechNodeConfigDto.cs`、`Scripts/Core/Config/ModifierTargetRegistry.cs`；**回滚说明**：`Config/TechTrees.json`、`Config/Buildings.json` 与 6 个受影响用例文件已 `git checkout` 复原，仓库保持 256/256 全绿 |
| `D110` | 2026-09-16 | **科技表落地：口径 + 两条已就绪的脚本 + 剩余 16 条的精确诊断（更正 `D109`）**。① **命名口径（用户裁定）**：树 Id = **`math`/`physics`/`chemistry`**（旧示例表的 `science` 是错的——数学不能叫 science；`military` 树设计稿里没有，直接删）；节点 Id **全部英文 slug**（93 条映射表已写进 `Tools/gen_tech.ps1`），中文名进 `Name`、效果原文进 `EffectText`。② **旧示例表不兼容、整表重生成**：`Tools/gen_tech.ps1` 解析 `design/research_tree.md` ⇒ 93 节点（math 30 / physics 35 / chemistry 28），跨树前置用 `"science:计数"` 式紧凑写法。③ **`Tools/migrate_tech_refs.ps1`**：把 6 个用例文件对旧表的引用一次性迁移（树名/节点名/样本形状/夹具 bypass），实跑 28 条替换全部命中（除 1 条无害的裸"军事树"字样），并自动插入 5 处"夹具落库根节点"。④ **剩余 16 条失败的精确成因（下一轮按此收口，不再试错）**：**(a) 断言里的旧 id** —— `ConfigTableChecks` 的"节点可检索"3 条与该文件的 `science/counting` Duration=0 白名单断言、`ConfigChecks` 跨表引用 fixture、`TechPrerequisiteChecks` 的 `science:counting` 白名单，全部改名 `math/*`；**(b) `UpgradeChecks.UnlockUpgradeTech` 只解了 3 个节点**，而建筑升级链现按设计稿要 8 个（camp←basic_geometry、camp_ii←preliminary_survey、school←pythagorean_school、school_ii←elements、workshop←basic_geometry、workshop_ii←preliminary_survey、military_camp←simple_machine_intuition、military_camp_ii←lever_balance）⇒ WP-2.6/WP-2.10 共 6 条"应能升级"失败；**(c) "夹具落库根"没生效** —— `h.Tech.GetOrCreateTechTree(...).Research("taming_of_fire")` 的域内标记被 app 服务的读路径忽略（与 `D25` 同类：读侧重载）⇒ WP-2.9 三条（树内串行/三树并行/任务键续跑）应改成**用 app 级 `h.Tech.Research(...)` 研究根节点 + 推进其天数**（根节点本身就是前置满足的，无需 bypass）；**(d)** `M0-2 物理树` 与服务 `science/counting` 白名单两条例外需按新树名各改 1 处 | ① 用户一句话纠了两个原则问题：**示例表不是契约（该删就删）**、**命名要按设计稿语义**（`science`≠数学）；② ③ 把"改 30+ 处引用"从手工变成可重跑脚本（并暴露了我用 shell 内联替换的坑：**引号/制表符在 PS 命令里非常容易自伤，涉及多行中文文本的批量改动一律写成 `.ps1` 文件**）；④ 诊断已细到"哪条断言、为什么"，下一轮预计 4~6 次工具调用即可收口 | `Tools/gen_tech.ps1`（slug 表 + math 树）、`Tools/migrate_tech_refs.ps1`（新）、`Scripts/TechTrees/Domain/TechPrerequisite*.cs`（注释举例改 `math:counting`）；**回滚说明**：本轮的 `Config/*.json` 与 6 个用例文件已 `git checkout` 复原，仓库保持 **256/256 + 冒烟 20/20** 全绿，表本体下一轮落地 |
| `D111` | 2026-09-16 | **B1 修正器底座口径（`WP-4.1` + `WP-4.4` + `WP-4.5`）**：① **阶段管道**：`ModifierStage{Building,Tech,Event}`，结算顺序 `result = base`，按阶段升序 `(result + ΣAbsolute) × (1 + ΣPercent)` ⇒ **与注册/加载顺序无关**，且 `ModifierValue.Stage` 缺省 `Building` ⇒ 旧档（无该字段）与旧公式**逐位一致**、170+ 处旧调用零改动；② **多 target 累加**（旧实现只取第一个命中的 target）：修掉"通用 `ResourceLimit` 被逐资源 `XxxLimit` 顶掉 ⇒ 科技上限加成静默失效"，并配套**同宿主同目标同类去重**（同一建筑两条相同修正不自我叠加）；③ **宿主作用域** `GetValueForHost(hostSourceId,…)`（Manager + AppService 两层）：只结算"该宿主挂的修正"，为 `WP-4.13` 产出归因铺路；④ **六个空转目标名全部接上消费者**：`BuildingSpeed`（建造+升级任务时长）、`BuildingUpgradeCost`（升级消耗）、`ResearchSpeed`（研究任务）、`UnitTrainingSpeed`（两处训练任务）、`UnitSpeed`（`MovementOf(unit, mapId)`；不传 mapId = 兼容路径不结算）、`FoodConsumption`（月结口粮需求）；耗时/造价类**同式结算**（`-30 日` 与 `-10%` 用同一公式，故 base 传"基础天数/造价"）；⑤ **`WP-4.5` 产出浮动**：`BuildingConfig.OutputVariance`（农田 lv.I ±20% / lv.II ±15% / lv.III ±10%，进表）、月结产出 `× (1 + roll)`、`roll ∈ [-lower, +upper]`；`OutputVarianceUpper` / `OutputVarianceLower` 一旦有修正器就**改写**该侧边界（观星台"上限 +5% / 下限 −1%"）；为此新增 `IOccupantQuery`（Map 模块实现，Resources 模块消费，避免模块方向绑死）+ `ModifierAppService.HasTarget`（区分"没修正器"与"修正器为 0"） | ① 阶段管道解决的是"同配置换加载顺序结果就变"这类**不可复现**问题（`N3` 复盘与自动化断言的前提）；② ③ 是"修机制顺手修掉一个真缺陷"；④ 一条 WP 让 **6 类设计稿效果**（建造/升级/研究/训练/移动/口粮）从"填了不生效"变成"填了真生效"，这是 B1 存在的理由；⑤ 观星台的范围部分（3 格内改写）本质是范围效果，归 `WP-4.2`，本 WP 只把**消费点**做好并留好修正器入口 | `Scripts/Common/Domain/ModifierStage.cs`（新）、`ModifierValue.cs`、`ModifierManager.cs`、`Scripts/Common/Application/ModifierAppService.cs`（stage/HasTarget/GetValueForHost）、`Scripts/Common/Domain/IOccupantQuery.cs`（新）、`ConstructionAppService`（`ScaledBuildDays`/`ScaledUpgradeCost`）、`UnitsAppService`（`ScaledTrainingDays`）、`UnitMovementService`（`MovementOf(unit, mapId)`）、`TechTreesAppService`（研究耗时）、`MonthlySettlementService`（口粮消耗 + `ApplyOutputVariance`/`MaxOutputVariance`/`BoundOrDefault`）、`BuildingConfigDto`/`IBuildingConfig`（`OutputVariance`）、`ModifierTargetRegistry`（`BuildingUpgradeCost`/`FoodConsumption`/`OutputVarianceUpper`/`OutputVarianceLower`）、`CoreBootstrap`（给 Units/Settlement 传修正器与建筑表/占据物查询）、用例 `ModifierConsumerChecks`（**6 条正向断言**：阶段顺序可复现、多 target/宿主作用域、建造 89/90 日界限、研究 7/8 日界限、训练 17/18 日界限、移动力与口粮需求 + 跨 owner 隔离、浮动范围与上下限改写） |
...
| `D116` | 2026-09-16 | **`WP-7.3` 科技表本体：本轮**生成器与迁移脚本已就绪但表未落地**（第二次回滚保绿），13 条失败的精确清单已定位**。① `Tools/gen_tech.ps1` 现在能产出**完整设计表**：**93 节点**（math 30 / physics 35 / chemistry 28，跨树前置如 `math:counting`、同树前置如 `taming_of_fire`）+ `Name`/`EffectText`/`UnlocksUi`（计数→资源面板+研究、算术→收获面板）+ 64 条映射到当前机制的 `Modifiers` + 29 条只留原文；② `Tools/migrate_tech_v2.ps1` 一次完成 12 个文件/112 处替换（AI 流派偏好、事件与建筑的科技前置重映射到设计稿『升级条件』列、6 个用例文件的树名/节点名、并发用例换 `chemistry` 样本 + **app 级探根**（域内直落会被读路径忽略，见 `D110`））；③ **剩余 13 条失败（下一轮按此清单逐条收，已定位到"哪条断言"）**：**(a)** `UpgradeChecks.UnlockUpgradeTech` 只解 3 个节点，而建筑升级链按设计稿要 **8 个**（camp←basic_geometry / camp_ii←preliminary_survey / workshop←basic_geometry / workshop_ii←preliminary_survey / school←pythagorean_school / school_ii←elements / military_camp←simple_machine_intuition / military_camp_ii←lever_balance）⇒ WP-2.6 ×4 + WP-2.10 ×1；**(b)** 并发用例的探根插入位置要在 **`CanResearch` 断言之前**（当前插在"开始研究"那行之前，断言先执行 ⇒ 前置仍不满足）⇒ WP-2.9 ×2（"pottery 前置满足"与"三树并行 ≤ 实际 1 条任务"）；**(c)** `M0-2 物理树` 与 `WP-1.4 跨表引用 fixture` 需要按新表名各改 1 处（`期望 3 实际 4` 是 fixture 树形状变了）；**(d)** `ModifierConsumerChecks` 的 `ResearchSpeed` 用例还用 `science/writing`（15 日）——换成 `chemistry:taming_of_fire`（10 日）并把界限改 4/5 日；**(e)** `InfoGateChecks` 的收获面板断言依赖 `算术` 的 `UnlocksUi`（生成器已写，但实测为空 ⇒ 下一轮查生成器的中文键匹配或改挂在 counting 上并登记） | 这一轮把"填 93 节点"从**手工抄写**变成了**可重跑的两步脚本**（生成 + 迁移），失败也从"未知"变成"13 条带定位"；② 顺带证实了 `D110` 的教训（域内直落无效、必须 app 级 + 推进天数）；**未落地不是设计冲突，只是预算用尽**：剩余 13 条预计 4~6 次工具调用可收口 | `Tools/gen_tech.ps1`（含 `UnlocksUi`）、`Tools/migrate_tech_v2.ps1`（新）；**回滚说明**：`Config/TechTrees.json`、`Config/AI.json`、`Config/Buildings.json`、`Config/Events.json` 与 6 个用例文件已复原，仓库保持 **276/276 + 冒烟 20/20** |
...
| `D122` | 2026-09-16 | **批次 5 收尾三连（`WP-5.6` 地图存档性能 · `WP-5.5` 迷雾性能 · `WP-5.4` 正式表现层）—— M2 至此全部完成**。① **`WP-5.6`（`MAP-07`/`MAP-08`）**：新增 `MapSaveMerger` —— 把"上次落盘的 DTO"留在仓储里，与本次映射结果**逐格比较**（地形/人口直接比字段，占据物比 DTO 序列化文本），未变格**复用旧 DTO**，脏格数精确可断言；`IMapRepository` 增 `LastSavedDirtyCells/LastSavedTotalCells/LastSaveSkippedWrite/LastSaveMilliseconds`（Godot 仓与内存替身同口径）；**脏格为 0 时整次跳过写盘**；应用层补 `DeleteMap`/`ListMaps`（此前只有"生成/读写"）。**口径**：物理文件仍"一张图一个 JSON"（`D53`）⇒ **有变化时文本仍是全量重写**，"增量"落在 DTO 复用与跳过写盘上，不假装是增量 IO。② **`WP-5.5`（`H1`/`FOG-03`/`FOG-04`）**：`FogGeometry` 把"半径内/半径环"的偏移量缓存（**模板构建次数与调用次数解耦**，用例直接断言 200 次揭图 0 次新构建）；`FogCodec` 用 zigzag varint + Base64 编码矩阵（~3 字节/格，旧 `"q,r": v` 约 12 字节/格），`FogSaveData` 增 `Compact`/`Encoding` 且**旧 `MatrixData` 仍可读**；`FogAppService.Save` 改为脏格增量 + 矩阵基线（`SavedMatrix`）+ 无变化跳过写盘（读档即建立基线 ⇒ 读档后立刻存档也是增量）。**口径**：多一份基线矩阵（~30 B/格内存）换增量与"读档后可增量"。③ **`WP-5.4`（`I2`）**：新增 `MapLayerModel`（**图层口径**：未探索格不出现在任何层；有建筑/单位的格进对应层；雾三档不透明度 1 / 0.55 / 0；占据物占位色表——甲方换美术只改颜色表）+ `MapInteractionModel`（**交互口径**：点选返回格信息（地形/占据物 uid/人口）；选址建造与单位指令**只提交 `PlayerIntent`**，成功即退出模式、失败留在模式并给 i18n 键；事件弹窗的 `PendingEvents`/`ConfirmEvent`；UI 门控查询转发 `UiGateService`）。Godot 侧只做最小接线：`MapView` 建 `TerrainLayer`/`OccupantsLayer` 容器 + `RefreshPresentation()`（雾与占据物逐格应用）+ `CellAtWorldPosition()`（`CellLayoutPosition` 的逆运算 + 邻居兜底）+ `_UnhandledInput`（左键=意图、右键/ESC=取消、`B`=建造营地占位入口）；`MapCellView` 加 `SetFogVisibility`（雾由格子自身 `Modulate` 承担——拆成上层节点会盖不住贴图）与 `SetOccupantMarker`（缩小 55% 的六边形标记）。**口径**：表现层的**可验证内核是纯 C#**（图层/交互 6 条无头用例），Godot 节点只剩"坐标换算 + 挂节点 + 把键交给 i18n"，因此 UI 无法绕过规则（`CON-02`/`CON-08`）。④ 复绿：`verify.ps1 -FullSmoke` = **headless 314/314 + 冒烟 20/20** | ① **M2 判据 ①②③ 与名单全部闭合**（批次 5 11/11、批次 6/7 全绿，M2 剩 0）；② 性能面有了"可断言的账"（脏格数 / 跳过写盘 / 紧凑字节数），后续改这些路径必须重跑用例；③ 表现层与美术解耦（占位图形 + 颜色表 + 图层容器），N4 到位后只替换贴图与颜色值 | `Scripts/Map/Infrastructure/MapSaveMerger.cs`（新）· `IMapRepository`/`GodotMapRepository`/`TestDoubles`（脏格统计）· `MapAppService`/`MapSession`（`DeleteMap`/`ListMaps`）· `Scripts/Fog/Domain/FogGeometry.cs`/`FogCodec.cs`（新）· `FogAppService`/`FogSaveData`（紧凑 + 增量）· `Scripts/Map/Presentation/MapLayerModel.cs`/`MapInteractionModel.cs`（新）· `MapView`/`MapCellView`/`DevMapUi`（最小接线）· `Tests/.../{MapSavePerfChecks,FogPerfChecks,PresentationChecks}.cs`（各 6 条） |
| `D123` | 2026-09-16 | **收尾两件（性能基线重跑 + `D121` 遗留的 `U7` 专项）—— 顺带挖出并修掉 3 个真 bug**。① **基线重跑**（`D83` 纪律：本批动了存档与渲染 ⇒ 必须重跑）：`Document/PerfBaseline.md` 更新到 v0.9.9 的实测（143×73：generate **229 ms** · start 35 ms · advance90 **577 ms** · save 77 ms · load 151 ms · render **1104 ms** · worldSave 12.2 KB · mapFile **2394.7 KB** · memStatic **142.5 MB** · memPeak 142.6 MB；24×24 快档同步更新），删掉已弃用的 73×143 旧行（`D83`：旧数字不保留），并把三条判读改写：`B1` 地图文件仍是磁盘大头（增量写**不缩文件**，要压体积得换编码）；`B2` 图层化**没有**增加节点数（仍是"一格一视图"，占据物标记懒建到个位数）；`B3` **逻辑不再"极便宜"**（90 日 38→577 ms，原因是 93 科技 / 20 事件 / AI 决策与寻路都上了日节拍，仍 < 1 s，但 `WP-6.x` 加 AI 数量/频率时要重测）。② **`D121` 遗留（读档后 AI 周期任务 +1）专项收口，挖出三个真 bug**：**(a)** `WorldSaveService.LoadWorld` 给**每个 owner** 都 `StartEventsEngine` ⇒ AI 被挂上事件引擎（多出一条 `EventTick`，且 AI 每次日节拍白跑事件掷骰）——违反 `G8`"事件只打断玩家的决策"；现按 `_session.IsHuman(owner)` 过滤（与 `StartMap` 同口径）。**(b)** `UnitMovementService.RegisterMoveLoop` 把任务 owner **硬写 0** ⇒ 任务表里出现"无主循环"（owner=0），按 owner 分组统计/过滤都会漏；现从单位自身取 owner。**(c)（最严重，本次新发现）** `AiService.StartEngine` 是幂等的（`_started.Add` 挡住重复挂），但 `LoadWorld` 的 `ITimeService.Reset()` 只清**时间轴订阅**、`_started` 仍记着"挂过了" ⇒ **读档后 `StartMap` 不再重挂 AI，AI 永远不再决策**（"僵尸 AI"）；现加 `AiService.ResetEngines(mapId)` + `SessionOrchestrator.ResetEngines` + `SessionEntryService.Load` 在读档后、`StartMap` 前调用。③ 用例从"容忍 ≥"改成**精确锁**：快照类任务读档后**逐条复原**（`SequenceEqual`）、**只允许新增单位循环**（`UnitMove`/`UnitAttack` 在存档里不是任务、按在场单位重建）、**事件引擎只挂人类**（`(a)` 的回归锁）、恢复条数仍 > 人类一方。④ 复绿：`verify.ps1 -FullSmoke` = **headless 314/314 + 冒烟 20/20** | ① `D83` 的"改存档/渲染必重跑基线"闭环；② `U7` 从"有回归锁 + 一条遗留"变成"**零遗留**，且带出 3 个真 bug 的回归锁"；③ 基线文档现在能解释"为什么 advance90 变慢"（内容与 AI 上线，不是实现退化） | `Document/PerfBaseline.md`（v0.9.9 实测 + B1/B2/B3 修订）· `Scripts/Core/Save/WorldSaveService.cs`（事件只挂人类）· `Scripts/Units/Application/UnitMovementService.cs`（owner 归属）· `Scripts/AI/Application/AiService.cs`（`ResetEngines`）· `Scripts/Core/SessionOrchestrator.cs`（转发）· `Scripts/Core/SessionEntryService.cs`（读档后作废）· `Tests/.../SessionEntryChecks.cs`（精确锁 + 失败消息带任务清单） |
| `D124` | 2026-09-16 | **`WP-8.1` 美术规格 + 清单定稿 + `WP-8.2` 接线（美术"丢文件即生效"）**。① **决策（N4 授权范围）**：**素材来源不影响代码**（谁画/AI 生成都能用）——只影响三件事：**授权/署名**（`AssetManifest` 的 `Owner`/`License` + `Credits.md`）、**风格一致性**（写死"正俯视偏 30° / 光源左上 / 朝向朝右下"）、**归一化成本**（规格钉死画布与锚点，附自动校验脚本）。规格由程序侧定义；**科技图标全画 93 张**；**BGM 1 首**（已有）；**音效暂缓**。② **清单与工具**：`Tools/gen_asset_manifest.ps1` 从四张表 + UI/音频规格**生成** `Document/AssetManifest.csv`（**189 行**：地形 5 · 建筑 24 · 单位 15 · 资源 3 · 事件 20 · 科技 93 · UI 28 · BGM 1）与 `Document/AssetList.md`（可直接发美术）；`Tools/check_assets.ps1` 自动验收（**缺图 / 尺寸 / 无透明通道 / 多余文件**，`-Promote` 把合格行升级 `delivered`）。**修掉两个真坑**：PowerShell 单元素数组会被展开 ⇒ 音频规格写成 5 条垃圾行（`$AUDIO_SPEC = ,@(...)` 修正）；`Terrains` 是**数组**、id 在 `Id` 字段 ⇒ 旧写法把 8 个 .NET 数组属性当地形登记（用例已加回归锁）。③ **规格定稿（`Document/ArtSpec.md` v0.9.10）**：七条硬规则（PNG-32 直通 alpha / 真透明无文字 / 画布=规格值 / 地形与图标 Center、**建筑与单位脚底贴下边** / 命名=表 id、路径不得改 / 统一视角光照 / 同类同画布）· 尺寸总表 · **地形图幅 352×406 → 366×423 更正**（= 列步长 × 行步长·4/3，六边形内切满画布、四角透明；旧值是"略小于步长"的早期猜测）· UI 28 清单 · 授权与 AI 生成记录口径 · 交付分包 01~07（最小可玩包 16 张）· 验收流程 · FAQ（含 `Texture/Terrain/test1-5.png` 历史测试图说明）。④ **接线（`WP-8.2` 的技术部分）**：`MapLayerModel` 增加路径常量与 `OccupantSpritePath/ResourceSpritePath/EventSpritePath/TechSpritePath/UiSpritePath`；`MapCellView` **有图用图、无图回退占位**（贴图按"宽 ≤ 格宽×0.9、高 ≤ 格高×0.9"**只缩不放大**，脚底对齐格内基准线）。⑤ **无头锁**：`PresentationChecks` +2 条（路径口径 / 清单闭合：8 个代表性路径 + 地形行数=5 + 行数≥189）⇒ **316/316 + 冒烟 20/20** | ① 美术**不需要读代码**就能开工（`AssetList.md` + `ArtSpec.md`），交付后我方一键验收；② "换美术不改代码"从承诺变成**可验证**（清单由表生成 + 路径用例锁住）；③ `WP-8.2` 只剩"收文件"，`WP-8.3`/`8.4` 是文案与打包 | `Tools/gen_asset_manifest.ps1`（新）· `Tools/check_assets.ps1`（新）· `Document/ArtSpec.md`（v0.9.10）· `Document/AssetList.md`（新，自动生成）· `Document/AssetManifest.csv`（47 → **189 行**，由脚本生成）· `Document/Credits.md`（新）· `Scripts/Map/Presentation/MapLayerModel.cs`（路径口径）· `Scripts/Map/Presentation/MapCellView.cs`（贴图接线 + 回退）· `Tests/.../PresentationChecks.cs`（+2 条） |
| `D125` | 2026-09-16 | **音效独立成册（`WP-8.4` 前置）：36 条音效 + 1 首 BGM 的清单与"音效契约"落地 —— 音频可即刻开工，且 id 与代码同源**。① **先说清事实**：此前**没有任何音频代码**（无播放器/无音量总线/BGM 未接）⇒ 所以不是"已经能响"，而是**把对接面钉死**：`Scripts/Audio/Domain/SoundCatalog.cs`（36 个音效 id + `main` BGM + 路径口径 `res://Audio/SFX/{id}.wav`、`res://Audio/BGM/{id}.ogg` + **8 类领域事件 → 音效**映射 + 意图成功/失败 → `ui_click`/`ui_reject`），播放器 `AudioAppService` 属 `WP-8.4`、等首批音频到位后接线（约一次小改）。② **为什么单独一套文档（用户要求：组内分工不同）**：图看 `ArtSpec.md`/`AssetList.md`，音看 `SoundSpec.md`/`SoundList.md`，**两份清单不重叠** —— `AssetManifest.csv` 由 189 行降为 **188 行（纯图）**，音频全部移交 `SoundManifest.csv`（**37 行** = 36 音效 + 1 BGM；P1 24 / P2 12）。③ **清单与工具**：`Tools/gen_sound_list.ps1` 从音效契约生成 `SoundManifest.csv` + `SoundList.md`（逐条给"文件 / 格式 / 建议时长 / **触发点（什么时候响）** / 优先级 / 说明" + **最小可玩音效包 9 条**）；`Tools/check_assets.ps1` 升级为**图 + 音一起验收**（图片尺寸/透明通道、音频扩展名、缺文件、目录里未登记的"多余文件"，`-Promote` 把合格行升 `delivered`；音频的**响度/时长**属人工抽查，已在文档写明）。④ **规格要点**：音效 `wav 44.1kHz/16-bit/单声道优先`（零延迟）、音乐 `ogg 44.1kHz 立体声无缝循环 60~120s`、峰值 ≈ -6 dBFS（BGM ≈ -14 LUFS）、**首尾不留静音**、**不做空间化**（同一音效可给 3 个变体）、**缺文件静默**（可边做边交）。⑤ **无头锁**：新增 `SoundChecks` 5 条 —— id 形态（小写+下划线=文件名）· 路径口径 · **8 类领域事件全覆盖**（新加事件忘了配音会红）· **清单与代码 id 闭合**（防漂移）· 空/未知 id 与缺文件不抛异常。⑥ 复绿：`verify.ps1 -FullSmoke` = **headless 321/321 + 冒烟 20/20** | ① 音频组拿到 `SoundList.md` 就能开工，**不需要看代码**；② "播放点"由代码决定并已被用例锁住，杜绝"文件都有就是不响"；③ 图/音两份清单分工清晰、都来自生成器，不会与各自的表/契约漂移；④ `WP-8.4` 只剩"播放器接线 + 打包" | `Scripts/Audio/Domain/SoundCatalog.cs`（新）· `Tests/.../SoundChecks.cs`（新，5 条）· `Tools/gen_sound_list.ps1`（新）· `Document/SoundManifest.csv`（新，37 行）· `Document/SoundList.md`（新，自动生成）· `Document/SoundSpec.md`（新）· `Tools/check_assets.ps1`（升级：图+音）· `Tools/gen_asset_manifest.ps1`（音频行移交）· `Document/ArtSpec.md`（音频指向 SoundSpec）· `Document/Credits.md`（音效署名行）· `Program.cs`（`Sound` 组） |
